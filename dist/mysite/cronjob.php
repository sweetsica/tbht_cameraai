...
$date=date('Y-m-d');
 ...
 foreach ($personNames as $personName) {
     ...
     $time_min=date('H:i:s', $min / 1000);
     $time_max=date('H:i:s', $max / 1000);
     ...
     where id_staff='".$personID[0]."' and id_room='".$deviceIDs[0]."'";
     if($query=mysqli_query($conn,$sql)){
        while($row =mysqli_fetch_assoc($query)){
        $sql1="Select * from timekeeping where date=".$date." and id_belong=".$row['id']."";
        $query1=mysqli_query($conn,$sql1);
        if(mysqli_num_rows($query1)>0){
            $sql3="UPDATE `cam_ai`.`timekeeping` SET `time_out`='".$time_max."' WHERE  `id_belong`=".$row['id'].";";
            if(mysqli_query($conn,$sql3)){
                $kq.= $date."lúc".$time_max."insert thành công";
            }
            else{
                $kq.= $date."lúc".$time_max."insert thất bại";
            }
        }
        else{
            ...
            $end_time = new DateTime(''.$time_min.'');
            ...
            $text=($time_min>"08:00:00")?"-":"+";
            $sql2="INSERT INTO `cam_ai`.`timekeeping` (`date`, `time_now`, `time_out`, `role`, `id_belong`) VALUES ('".$date."', '".$time_min."', '".$time_max."', '".$text."".$time."', '".$row['id']."');";
            if(mysqli_query($conn,$sql2)){
                $kq.= $date."lúc".$time_min."insert thành công";
            }
            else{
                $kq.= $date."lúc".$time_min."insert thất bại";
            }
        }    
    }
     }
